...
#define REGIONGROWING_TYPE 2
  ...
  public:
    //registration下采样大小
    float leaf_size_registration;
    //icp参数
    float icp_transformation_epsilon;
    float icp_max_correspondence_distance;
    float icp_euclidean_fitness_epsilon;
    int icp_max_iterations;
    int icp_optimized_iterations;
    int need_global_registration;
    //ndt参数
    float ndt_resolution;
    float ndt_step_size;
    float ndt_transformation_epsilon;
    int ndt_max_iterations;
    // 配准截取区域
    float clip_region_min_x;
    float clip_region_min_y;
    float clip_region_max_x;
    float clip_region_max_y;
    int reg_model;
  };
